...
import 'package:ycapp_bloc/bloc/data_blocs/mobile/boo_pref_mobile.dart';
import 'package:ycapp_bloc/bloc/data_blocs/mobile/confic_bloc_mobile.dart';
...
class SettingsBlocMobile {
  ...
  SettingsBlocWeb() {
    remoteConfig = ConfigBlocMobile();
    boolPref = BoolPrefMobile();
    intPref = IntPref();
    doublePref = DoublePref();
    stringPref = StringPref();
    stringListPref = StringListPref();
  ...
  Future<bool> init() async {
    await time('init()', () async {
      ...
      await Future.wait([
        time('DeviceInfoPlugin', () async {
          DeviceInfoPlugin deviceInfo = DeviceInfoPlugin();
          androidInfo = await deviceInfo.androidInfo;
          print('androidInfo $androidInfo');
          boolPref.androidInfo = androidInfo;
          return null;
        }),
        time('_init()', () async {
          await _init();
...
class SettingsProviderMobile extends StatefulWidget {
  final Widget child;

  SettingsProviderMobile({Key key, @required this.child}) : super(key: key);

  @override
  _SettingsState createState() => _SettingsState();

  static SettingsBlocMobile of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<_SettingsProvider>().bloc;
  }
}

class _SettingsState extends State<SettingsProviderMobile> {
  SettingsBlocMobile bloc;
